...
filepath = 'frontend/src/dashboard/projects/ProjectDetailsPage.jsx'

with open(filepath, 'r', encoding='utf-8') as f:
    text = f.read()

# 1. Add getViewWeeks helper
helpers_replacement = """
...
function buildWeekDescriptor(mondayDate) {
    ...
    let label = isCurrent ? "This Week" : `Week ${wkNum}`;
...
function getViewWeeks(mode, startStr, endStr) {
    ...
    while (cur <= endMonday && weeks.length < 200) {
...
# Replace lines 11 to 62
text = re.sub(r'function getMonday\(date\).*?const WEEK_DEFAULT_HOURS = 40;', helpers_replacement, text, flags=re.DOTALL)

# 2. Fix normalizeAllocationRow
norm_replacement = """
function normalizeAllocationRow(row = {}) {
    const normalized = { weekly_hours: {}, ...row };
    // Keep raw weekly_hours dict intact, we just ensure it exists
...
"""
text = re.sub(r'function normalizeAllocationRow.*?function getResourceAllocationPct\(row\) \{.*?\n\}', norm_replacement, text, flags=re.DOTALL)

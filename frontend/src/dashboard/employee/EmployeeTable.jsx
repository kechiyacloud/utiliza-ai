...
import BenchPool from './insights/BenchPool';
...
const EmployeeTable = ({ onEmployeeClick, filters }) => {
    ...
    const [showBenchPool, setShowBenchPool] = useState(false);
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-full">
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-100 flex-shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    {filters?.cardFilter === 'bench' && showBenchPool && (
                        <span className="text-xs font-bold px-2 py-0.5 bg-orange-50 text-orange-600 rounded">Upcoming Bench</span>
                    )}
                </div>
                <div className="flex items-center gap-4">
                    {filters?.cardFilter === 'bench' && (
                        <button
                            onClick={() => setShowBenchPool(!showBenchPool)}
                        ...
                        >
                            {showBenchPool ? 'Back to Standard Directory' : 'View Upcoming Bench'}
                        </button>
                    )}
                    {!showBenchPool && (
            ...
            {/* Content Area */}
            {showBenchPool && filters?.cardFilter === 'bench' ? (
                <div className="flex-1 overflow-y-auto custom-scrollbar h-[500px] max-h-[60vh] bg-slate-50/30">
                    <BenchPool employees={filteredEmployees} />
                </div>
            ) : (
                <div className="flex-1 overflow-y-auto custom-scrollbar h-[500px] max-h-[60vh]">

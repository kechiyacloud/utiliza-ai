...
const EmployeeMonthCard = ({ onClick }) => {
    ...
    return (
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-start relative min-w-[150px] h-full flex-1 cursor-pointer group hover:-translate-y-1 transition-transform duration-300 overflow-hidden" onClick={() => onClick && onClick(employee)}>
            ...
            <div className="flex w-full items-start justify-between z-10 mb-3">
                <div className="p-2 bg-slate-50 border border-slate-100 rounded-xl text-slate-500 group-hover:bg-purple-50 group-hover:text-purple-600 group-hover:border-purple-200 transition-colors">
                    <Trophy size={20} strokeWidth={2.5} />
                </div>
                {employee.photo_url ? (
                    <img src={employee.photo_url} alt="Profile" className="w-8 h-8 rounded-full border-2 border-white shadow-sm object-cover" />
                ) : (
                    <div className="w-8 h-8 rounded-full border-2 border-white shadow-sm bg-purple-100 flex items-center justify-center text-purple-700 text-[10px] font-bold">
            ...
            <div className="relative z-10 flex flex-col justify-end flex-1 w-full text-left">
                <h3 className="text-xl font-bold text-slate-900 tracking-tight group-hover:text-purple-600 transition-colors truncate mb-1">{employee.employee_name}</h3>
                <p className="text-slate-500 text-[10px] font-bold tracking-wider uppercase mb-1">Employee of the Month</p>
                <p className="text-[10px] text-slate-400 font-medium truncate">{employee.role_designation}</p>
            </div>
        </div>
    );
};
